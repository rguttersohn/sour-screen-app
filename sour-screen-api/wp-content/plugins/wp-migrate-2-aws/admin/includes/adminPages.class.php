...
class WPM2AWS_AdminPages
{
            ...
            if (defined('WPM2AWS_LIMIT_ZIP_DIR_SIZE') && true === WPM2AWS_LIMIT_ZIP_DIR_SIZE) {
                ...
                if (!empty($scannedFs)) {
                    ...
                    if (!empty($excludedDirs)) {
                        ...
                        $html .= '</ul>';
                        $html .= '<p><em>Please contact <a href="mailto:info@seahorse-data.com">Seahorse Data</a> if required.</em></p>';
                        $html .= '</div>';
                        $html .= '</div>';
                    } else {
    ...
    private static function makeConfirmedLightsailInfoSection()
    {
        if (get_option('wpm2aws-lightsail-instance-details') !== false) {
            ...
            echo '<h4>Your instance is currently being compiled and will be available shortly. An email will be sent to the email address associated with your licence key with further information and active links. The details outlined below are for reference only.</h4>';
            echo '<ul>';
            if (defined('WPM2AWS_TESTING')) {
            ...
            /// show this to the review user
             if (
                ...
                echo '<p>At this point it will be shut-down unless users choose to purchase a plan for ongoing WordPress Hosting on AWS</p>';

                    echo '<ul>';
                    echo '<li>Tier 1: <a target="_blank" href="https://www.seahorse-data.com/checkout?edd_action=add_to_cart&download_id=1330&edd_options[price_id]=2">Add to Cart</a></li>';
                    echo '<li>Tier 2: <a target="_blank" href="https://www.seahorse-data.com/checkout?edd_action=add_to_cart&download_id=7318&edd_options[price_id]=2">Add to Cart</a></li>';
