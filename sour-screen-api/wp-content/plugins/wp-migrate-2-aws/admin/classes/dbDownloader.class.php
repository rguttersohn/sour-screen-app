...
class WPM2AWS_DbDownloader
{
    ...
    public function init()
    {
        ...
        if ($dbSize > WPM2AWS_MAX_DB_EXPORT) {
            ...
            if (empty($response)) {
                ...
                wpm2awsLogRAction('wpm2aws-run-db-download', 'Success! DB Download Complete');
    ...
    public function exportFullDb()
    {
        ...
        $responseVals = get_object_vars($response);
        if (empty($responseVals['status'])) {
            $responseVals['status'] = false;
        }
        if (empty($responseVals['message'])) {
            $responseVals['message'] = 'Could Not Determine Response Message';
        }


        if ($responseVals['status'] === true) {
            wpm2awsAddUpdateOptions('wpm2aws_download_db_complete', 'success');
            wpm2awsLogRAction('wpm2aws-run-db-download', 'wpm2aws_download_db_complete (fdb): Success');
            set_transient('wpm2aws_admin_success_notice_' . get_current_user_id(), __('Success!<br><br>Prepare Database Complete.', 'migrate-2-aws'));
            wpm2awsAddUpdateOptions('wpm2aws_current_active_step', 3);
        } else {
            wpm2awsLogAction('Error! DB Download Failed - Bad Response: ' . $responseVals['status'] . ' | '  . $responseVals['message']);
            wpm2awsLogRAction('wpm2aws-run-db-download', 'wpm2aws_download_db_complete (fdb): Error: ' . $responseVals['status'] . ' | '  . $responseVals['message']);
            wpm2awsAddUpdateOptions('wpm2aws_download_db_complete', 'error');
            set_transient('wpm2aws_admin_error_notice_' . get_current_user_id(), __('Error!<br><br>Prepare Database Failed. <br>Error Ref: ' . $responseVals['message'], 'migrate-2-aws'));
        }
        exit(wp_safe_redirect(admin_url('/admin.php?page=wpm2aws')));
    }
    ...
    public function exportDbTable($tableName)
    {
        ...
        //     wpm2awsAddUpdateOptions('wpm2aws_current_active_step', 3);
        // } else {
        //     wpm2awsLogAction('Error! DB Download Failed - Bad Response: ' . $response->status . ' | '  . $response->message);
        //     wpm2awsAddUpdateOptions('wpm2aws_download_db_complete', 'error');
        //     set_transient('wpm2aws_admin_error_notice_' . get_current_user_id(), __('Error!<br><br>Prepare Database Failed. <br>Error Ref: ' . $response->message, 'migrate-2-aws'));
        //     exit(wp_safe_redirect(admin_url('/admin.php?page=wpm2aws')));
        // }
        ...
        if ($responseVals['status'] !== true) {
            wpm2awsLogAction('Error! DB Download Failed - Bad Response: ' . $responseVals['status'] . ' | '  . $responseVals['message']);
            wpm2awsLogRAction('wpm2aws-run-db-download', 'Error! DB Download Failed - Bad Response: ' . $responseVals['status'] . ' | '  . $responseVals['message']);
            wpm2awsAddUpdateOptions('wpm2aws_download_db_complete', 'error');
            set_transient('wpm2aws_admin_error_notice_' . get_current_user_id(), __('Error!<br><br>Prepare Database Failed. <br>Error Ref: ' . $responseVals['message'], 'migrate-2-aws'));
            exit(wp_safe_redirect(admin_url('/admin.php?page=wpm2aws')));
        }

        return $responseVals['status'];
    }
