...
class WPM2AWS_Console
{
    ...
    public function addMetricAlarm()
    {
        ...
        $timeValue = ($hoursMinsValue/5) + round($minutesValue/5);
     ...
     * API Wrapper Function
     */
    public function createAlarm($name, $metric, $comparitor, $triggerPoint, $threshold)
    {
        $apiGlobal = new WPM2AWS_ApiGlobal();
        // $alarmName, $metric, $comparitor, $triggerPoint, $threshold
        try {
            $alarm = $apiGlobal->remoteAddNewMetricAlarm($name, $metric, $comparitor, $triggerPoint, $threshold);
