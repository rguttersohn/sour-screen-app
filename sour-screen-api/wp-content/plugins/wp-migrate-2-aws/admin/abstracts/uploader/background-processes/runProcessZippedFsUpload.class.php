...
class WPM2AWS_RunProcessZippedFsUpload extends WP_Background_Process
{
     ...
     * @var string
     */
    protected $action = 'wpm2aws-zipped-fs-uploader-all';
    /**
     * Task
     *
    ...
    protected function task($item)
    {
        ...
        if (defined('WPM2AWS_TESTING') || defined('WPM2AWS_DEBUG') || defined('WPM2AWS_DEV')) {
            $this->log("Zipped FS Upload Start: " . $item);
        }
        
        // Sleep for 2seconds after every 5% of file uploads complete
        $this->pauseProcess();
        
        // Run the "Upload Or Directory" Process
        $uploaded = $this->backgroundUploadToS3($item, 'zipped-fs-upload');
    
        // Dev / Testing Logging
        if (defined('WPM2AWS_TESTING') || defined('WPM2AWS_DEBUG') || defined('WPM2AWS_DEV')) {
            $this->log("Zipped FS Upload Result (final): " . $uploaded);
        }
        ...
        if ((string)$uploaded !== '200' && (string)$uploaded !== '404') {
            ...
            */
            if (true === $permenantFail) {
                ...
                $this->log("Zipped FS Upload Failed - Returning to Background Queue");
                return $item;
            }
        }
        ...
        */
        if ((string)$uploaded === '200') {
            if (defined('WPM2AWS_TESTING') || defined('WPM2AWS_DEBUG') || defined('WPM2AWS_DEV')) {
                $this->log("Zipped FS Upload Success");
            }
        } else if ((string)$uploaded === '404') {
            $failedUserNotice = '';
            if (false !== get_option('wpm2aws_zipped_fs_upload_failures')) {
                $failedUserNotice = get_option('wpm2aws_zipped_fs_upload_failures');
            }
            ...
            // Add to register
            wpm2awsAddUpdateOptions('wpm2aws_zipped_fs_upload_failures', $failedUserNotice);

            // Log Failure
            $this->log("Zipped FS Upload Failed (404)");
        } else {
            $this->log("Zipped FS Upload Failed (unknown) - " . $uploaded);
            wpm2awsLogRAction('wpm2aws_fs_uploader_fail', "Zipped FS Upload Failed (unknown) - " . $zipped);
        }
    ...
    protected function complete()
    {
        ...
        // $this->log('TempDirectory - ' . $zipTempDirectory);
        ...
        $this->log('Zipped FS Upload Process Complete - ' . date("d-m-Y @ H:i:s"));
        wpm2awsLogRAction('wpm2aws_fs_uploader_complete', 'Zipped FS Upload Process Complete - ' . date("d-m-Y @ H:i:s"));
        
        wpm2awsAddUpdateOptions('wpm2aws_zipped_fs_upload_complete', 'error');
        if (false === get_option('wpm2aws_zipped_fs_upload_errors') || empty(get_option('wpm2aws_zipped_fs_upload_errors'))) {
            wpm2awsAddUpdateOptions('wpm2aws_zipped_fs_upload_complete', 'success');
            wpm2awsAddUpdateOptions('wpm2aws_current_active_step', 5);
        }

        delete_option('wpm2aws_zipped_fs_upload_process_start_time');
    }
     ...
     *
     * @param string $uploadedItem The path to item being uploaded
     * @param int $maxAttempts Number of allowed attempts - default = 5
     * @return boolean
     */
    protected function isPermenantFail(string $uploadedItem, $maxAttempts = 5)
    {
        ...
        if (false === get_option('wpm2aws_bgProcessAttempts')) {
            ...
            $attemptsUpdate = array(
                $uploadedItem => $attempts
            );
            wpm2awsAddUpdateOptions('wpm2aws_bgProcessAttempts', $attemptsUpdate);
            // Return to queue
            $this->log("Returning item ( " . $uploadedItem . " ) to Queue - attempt: " . $attempts);
            return false;
        } else {
            ...
            if (isset($failedUploads[$uploadedItem])) {
                $attempts = $failedUploads[$uploadedItem];
            ...
            $failedUploads[$uploadedItem] = $attempts;
            if ($attempts < 1) {
                unset($failedUploads[$uploadedItem]);
            }
            ...
            if ($attempts > 0 && $attempts <= $maxAttempts) {
                ...
                // remove item from queue
                $this->log("Item is considered a permenant fail - " . $uploadedItem);
                wpm2awsLogRAction('wpm2aws_fs_uploader_error', "Item is considered a permenant fail - " . $uploadedItem);
    ...
    protected function updateProgressCounter()
    {
        ...
        update_option('wpm2aws_zipped_fs_upload_counter', $update);
        return;
    }
