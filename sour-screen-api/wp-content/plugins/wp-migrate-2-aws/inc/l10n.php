...
function wpm2aws_l10n() {
	...
	$api = translations_api( 'plugins', array(
		'slug' => 'migration-2-aws',
		'version' => WPM2AWS_VERSION,
	) );
...
function wpm2aws_is_valid_locale( $locale ) {
	$pattern = '/^[a-z]{2,3}(?:_[a-zA-Z_]{2,})?$/';
	return (bool) preg_match( $pattern, $locale );
}

function wpm2aws_is_rtl( $locale = '' ) {
...
function wpm2aws_load_textdomain( $locale = null ) {
	global $l10n;

	$domain = 'migration-2-aws';
